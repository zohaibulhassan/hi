...
class ManageUsersController extends Controller {
    ...
    public function activeUsers() {
        $pageTitle    = 'Manage Active Customers';
    ...
    public function detail($id) {
        ...
        $order['total_orders']       = Order::where('user_id', $user->id)->count();
        $order['total_shopping']    = Order::delivered()->where('user_id', $user->id)->sum('total');
        $order['total_transactions'] = Transaction::where('user_id', $user->id)->count();
        $countries                  = json_decode(file_get_contents(resource_path('views/partials/country.json')));
        return view('admin.users.detail', compact('pageTitle', 'user', 'order', 'countries'));
    }
