...
$(".custom-data-table")
  .closest(".card")
  .prepend(
    '<div class="card-header" style="border-bottom: none;"><div class="text-right"><div class="form-inline float-sm-right bg--white"><input type="text" name="search_table" class="form-control" placeholder="Search"></div></div></div>'
  );
$(".custom-data-table")
  .closest(".card")
  .find(".card-body")
...
function modalStatus(status, elementId) {
  ///====== type=1=edit
  if (status == 1) {
    $(elementId).attr("checked", true);
    $(elementId).closest(".toggle").removeClass("btn btn--danger off");
    $(elementId).closest(".toggle").addClass("btn btn--success");
    console.log(status);
  } else {
    $(elementId).attr("checked", false);
    $(elementId).closest(".toggle").removeClass("btn btn--success");
    $(elementId).closest(".toggle").addClass("btn btn--danger off");
  }
}
